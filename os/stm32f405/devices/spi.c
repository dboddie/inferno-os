...
void spi_set_frequency_divider(int divider)
{
    SPI *spi1 = (SPI *)SPI1;
    /* Increase the baud rate of transfers to the screen. */
    uint old = spi1->cr1 & ~(7 << SPI_br_shift);
    spi1->cr1 = old | ((divider & 7) << SPI_br_shift);
}
...
int spi_wait_for_response(int n, int delay_ms)
{
    /* Send padding bits. */
    int res = 0xff;
    int t = 0;
